...
%% Look for trials that were too early 

if length(Begin) ~= length(End) % if you find more start messages than end, some of the trial where repeated
    message = Data.Events.Messages.info(Begin_v); % Get the messages
    for aa = 1: length(message) % loop through them to find double ones
        if aa == length(message) % The last message cannot be a double
            Double(aa) = 0;
        else
            if strcmp (message(aa),message(aa+1)) % If you find the same one
                Double(aa) = 1;
            else
                Double(aa) = 0;
            end
    ...
    index = find(Double == 0); % Look for the single ones
    Begin = Begin(index); % Update the variable
end
...
cd('C:\src\FRL\PeripheryPerceptEye\Contrast\');