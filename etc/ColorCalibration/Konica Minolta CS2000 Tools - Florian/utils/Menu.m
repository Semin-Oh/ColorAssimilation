        ...
        function varargout = basic(varargin)
            ...
            if ischar(c)                                                    %single item
                ...
                while isempty(userInput)
                    %wait until keyboard buffer is empty to avoid 
                    %unintentional selection / confirmation
                    t0 = GetSecs; 
                    sec_passed = 0;
                    keyHit = true;
                    while keyHit
                        [keyHit, t, key] = KbCheck;
                        if keyHit
                            if floor(t - t0) > sec_passed
                                keycode = find(key);
                                nKey = numel(keycode);
                                warning(sprintf(['Check your ' ...
                                    'keyboard. Continuous key press ' ...
                                    '(%d', repmat(', %d', ...
                                    [1, nKey - 1]), ') detected.'], ...
                                    keycode));                              %#ok          
                                sec_passed = floor(t - t0);
                            end
                        end
                    end                                      
                    
                    if flag_numeric, userInput = Menu.numericInput(prompt);
                    else, userInput = input(sprintf('%s', prompt), 's');
                    end
                    
                    if isempty(userInput)
                        if ~isempty(default)
                            userInput = default;
                            if flag_numeric, fprintf('\b%d\n', default);
                            else, fprintf('\b%s\n', default);
                            end
                        elseif flag_empty
                            break;
