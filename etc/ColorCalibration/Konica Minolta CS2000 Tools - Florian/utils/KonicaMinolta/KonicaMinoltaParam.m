    ...
    methods
        ...
        function x = isValid(obj, value)
            ...
            if Misc.isCellOf(value, 'char') || ischar(value)
                ...
                minInt = obj.isOneBasedIndex;
                x = Misc.is(value, 'int', ...
                    [minInt, numel(obj.valid) - (1 - minInt)]);                
            end
        end
        ...
        function x = toChar(obj, value)
            ...
            if isnumeric(value)
                ...
                if n > 1
                    ...
                    x = obj.valid{value + ~obj.isOneBasedIndex}; 
                end
            else
                x = value;
            end
        ...
        function x = isOneBasedIndex(obj)
            %isOneBasedIndex returns true if device uses one-based indices (false =
            %zero-based).
            %
            %   Output: logical scalar
            
            if contains(class(obj), 'CM700'), x = 1;                                %CM700 parameters are one-based indices
            else, x = 0;                                                            %CS-2000 parameters are zero-based indices
            end
        end
    end
end
