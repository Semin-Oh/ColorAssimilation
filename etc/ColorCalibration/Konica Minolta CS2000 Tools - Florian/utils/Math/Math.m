classdef Math
    ...
    methods (Static)
        ...
        function x = gabor(xy_deg, cycPerDeg, sigma_deg, amp, ...
            ...
            x = amp * Math.gauss(d, sigma_deg) .* ...
                squeeze(cos(cycPerDeg * 2 * pi * ...
        ...
        function y = gauss(x, sigma, normalization)
            ...
            %               none        not normalized (def, 1 at x = 0)
            %               sum         Output sum is 1
            ...
            valid = {'sum', 'continuous', 'none'};
            if nargin < 3, normalization = 'none'; end
            if ~isfloat(x)
