classdef (Abstract) Misc
    ...
    %       pseudoRand          Returns int array (pseudo-rand. indices)
    ...
    methods (Static)
        ...
        function b = isValidFilename(x, checkExtension)
            %isValidFilename returns true if a char array is a valid 
            %filename, i.e. does not contain forbidden characters.
            %
            %   Input:  char array
            %           logical scalar (true = check extension exists, 
            %               def = false)
            %   Output: logical scalar
            
            if nargin < 2, checkExtension = false; end
            if ~Misc.is(checkExtension, 'logical', 'scalar')
                error('Second parameter must be a logical scalar.');
            end
            
            b = ischar(x) && ~isempty(x) && ...
                ~any(ismember('/\?%*:|"<> ', x));
            if b && checkExtension
                i = find(x == '.', 1, 'last');
                b = ~isempty(i) && i > 1 && i < numel(x);
            end
        end
        ...
        function [x, iRand] = pseudoRand(x, nMaxRepeat, iStart)
            %pseudoRand returns randomized integers with a limited number 
            %of successive repetitions on one number.
            %
            %   Input:  numeric array (values to pseudo-randomize)
            %           int scalar (number of max. repeat)
            %           int scalar (index from which randomization starts, 
            %               def = 1)
            %   Output: numeric array (pseudo-randomized values)
            %           int array (randomization index)
            
            if nargin < 3, iStart = 1; end
            if ~(isnumeric(x) && ~isempty(x))
                error(['First parameter must be a non-empty numeric ' ...
                    'array.']);
            elseif ~Misc.is(nMaxRepeat, 'int', 'scalar', 'pos')
                error('Second parameter must be a positive int scalar.');
            elseif ~Misc.is(iStart, 'int', 'scalar', {'<', numel(x)})
                error('Third parameter must be an int scalar < %d.', ...
                    numel(x));
            end
            ...
            while ~done
                idx = iStart : nSeq;                                        %indices of posSeq and iValueSeq to be randomized
                if randomize
                    endIsOk = false;
                    while ~endIsOk
                        iRand(idx) = idx(randperm(numel(idx)));              %randomized indices of idx
                        x(idx) = x(iRand(idx));
                        endIsOk = numel(unique( ...
                            x(end - (0 : nMaxRepeat)))) > 1;                %check if the last nMaxRepeat + 1 entries are not unique
                    end
                end      
                ...
                iStart_ = nSeq;
                for iuqx = 1 : nuqx
                    valid = x(idx) == uqx(iuqx);
                    invalid = valid(1 : end - nMaxRepeat - 1);
                    for iRepeat = 1 : nMaxRepeat
                        invalid = invalid & valid(iRepeat + 1 : ...
                            end - nMaxRepeat + iRepeat - 1);
                    end
                    i0 = find(invalid, 1, 'first') + nMaxRepeat + ...
                        iStart - 1;                                         %index of first entry that is repeated to often
                    if ~isempty(i0)
                        done = false;
                        i1 = find(x(i0 : end) ~= uqx(iuqx), 1, ...
                            'first') + i0 - 1;
                        if ~isempty(i1)
                            x([i0, i1]) = x([i1, i0]);                      %swap elements
                            iRand([i0, i1]) = iRand([i1, i0]);
                            if i0 < iStart_, iStart_ = i0; end
                        else
                            randomize = true;
                            iStart_ = iStart;                               %re-randomize complete sequence
                            break;
                        end
                    end
                end
                
                if ~done, iStart = iStart_; end
            end
        end
        ...
        function [key, t] = waitForKey(keys)
            ...
            %           float scalar (system time of key press)
            ...
            while ~done
                ...
                if keydown
                    key = uint8(find(keycode, 1, 'first'));
                    if nargin == 1
                        done = any(key == keys(:));
                    else
                        done = true;
