classdef Peak
    %Peak encapsulates a spectral peak.
    %
    %   properties
    ...
    %
    %   static methods
    %       fromSpectrum    Returns Peak array
    ...
    methods (Static)
        function obj = fromSpectrum(spectrum, minValue)
            %fromSpectrum returns a Peak array from a Spectrum object.
            %
            %   Input:  Spectrum object
            %           float scalar (minimal FWHM, def = 5)
            %           float scalar (minimal value at peak, def = max)
            %   Output: Peak array
            
            if ~Misc.is(spectrum, 'Spectrum', 'scalar')
                error('First parameter must be a Spectrum object.');
            end
            if nargin < 2, minValue = max([spectrum.mean.value]); end
            if ~Misc.is(minValue, 'float', 'pos', 'scalar')
                error('Second parameter must be a positive float scalar.');
            end
