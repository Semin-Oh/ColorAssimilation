classdef (Abstract) Color
    ...
    methods(Static)
        ...
        function R = getTransformationMatrix(x1, x2)
            %getTransformationMatrix retuns the matrix required to 
            %transform between color matching functions (cmf) and / or cone 
            %fundamentals (cf), e.g. from XYZ to SML), so that R * x1 = x2.
            %
            %   Input:  Spectrum or CMF or CRF object (original cmf / cf)
            %           Spectrum or CMF or CRF object (original cmf / cf)
            %   Output: 3 x 3 matrix
          
            x = {x1, x2};
            for i = 1 : 2
                if Misc.is(x{i}, 'CMF', 'scalar')
                    x{i} = x{i}.spectrum;
                elseif Misc.is(x{i}, 'CRF', 'scalar')
                    x{i} = Spectrum(x{i}.spectrum.wavelength, ...
                        x{i}.spectrum.value(:, [1, 3, 4]));
                elseif ~(Misc.is(x{i}, 'Spectrum', 'scalar') && ...
                        x{i}.count == 3)
                    error(['%s parameter must be a CMF, a CRF, or a ' ...
                        'Spectrum object with a count of 3.'], ...
                        Misc.ordinalNumber(i));
                end
            end
            
            x = Spectrum.commonDomain([x{1}, x{2}]);
            R = x(2).value' / x(1).value';
        end
