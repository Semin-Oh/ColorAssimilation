...
load('SPECTRA_Samsung wide_left_combined_01-Aug-2022.mat') %the dates for these two files should be the same
filetosave = 'calibration_Samsung wide_left_combined_01-Aug-2022.mat';
...
%         hold on
        ...
        plot(wl,spectrum,'k-','color',cols(ch,:).*val/255)
    end
end
...
%     subplot(2,4,ch)
    ...
    scatter(0:255,xyY(:,ch,1),10,cols(ch,:))
    ylabel('x')
    xlabel('intensity')
%     subplot(2,4,ch+4)
    subplot(1,2,2); hold on
    scatter(0:255,xyY(:,ch,2),10,cols(ch,:))
    ylabel('y')
    xlabel('intensity')
end
...
for ch=1:4
    LUMnew(:,ch) = fittedmodel{ch}(inputVmain)*255;
end
