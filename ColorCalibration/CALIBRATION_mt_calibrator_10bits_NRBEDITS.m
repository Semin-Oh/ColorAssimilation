...
%%%% try measuring zero
            ...
            Screen('FillRect',window,zeros(1,3),StimRect)
...
for ch=1:4
    for v= 1:length(values)
        if ch <= 2 %Change these lines and the else/end part to only run specific channels
            ...
            Screen('FillRect',window,gray);
            Screen('FillRect',window,col,StimRect)
            ...
            time=clock;
            TimeStamps{ch}{v} = datetime;
            save(sprintf('SPECTRA_%s_%s',room,string(date)),'SPECTRA','intercept','time','TimeStamps')
            %         sound(snd,40000);
        end
    end
end
