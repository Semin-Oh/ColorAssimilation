classdef Spectrum < matlab.mixin.Copyable
    ...
    methods (Static)
        ...
        function spectrum_ = commonDomain(spectrum, method)
            ...
            %find common range
            minwvl = spectrum_(1).wavelength(1);
            maxwvl = spectrum_(1).wavelength(end);
            for i = 2 : n
                minwvl = max(spectrum_(i).wavelength(1), minwvl);
                maxwvl = min(spectrum_(i).wavelength(end), maxwvl);
            end
            ...
            wvl = cell(1, n);
            for i = 1 : n
                valid = spectrum_(i).wavelength >= minwvl & ...
                    spectrum_(i).wavelength <= maxwvl;
                wvl{i} = spectrum_(i).wavelength(valid);
            ...
            for i = 1 : n
                if ip_necessary
                   spectrum_(i).interpolate(ip_step, method);
                end
                spectrum_(i).crop([minwvl maxwvl]);
            end
        end
    end
    ...
    methods (Static, Hidden)
        ...
        function checkIpMethod(x, i)
            %checkIpMethod throws an error if the given sting does not
